...
const createJwt=(payload,type)=>{
    ...
    else if(type===user) token=jwt.sign({payload},process.env.ACCESS_TOKEN,{
        expiresIn: process.env.LIFETIME,
    });

    else if(type===verifyEmail) token=jwt.sign({payload},process.env.VERIFY_EMAIL,{
        expiresIn: process.env.LIFETIME,
    });
...
const tokenValid=(token,type)=>{
    ...
    else if(type===verifyEmail) isValid=jwt.verify(token,process.env.VERIFY_EMAIL);
